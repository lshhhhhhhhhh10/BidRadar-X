...
AI 能力采用后端可插拔设计：未配置密钥时继续使用原有规则链路，配置智谱 API Key 后自动启用需求理解、检索扩词、检索规划、相关性复核、歧义去重、事实核验和证据化 Word 摘要。完整说明见 [docs/AI_PIPELINE.md](docs/AI_PIPELINE.md)。
...
| R05 登录来源 | 已实现适配器，待配置凭据 | 天眼查招投标 API、SAM.gov API | 缺用户 Token/API Key 时不会进入生产路由 |
