...
> 2026-07-15 已将可信恢复历史提升为默认 `main`。原远端基线和 C01 交接点分别保留在 `archive/application-baseline-20260713` 与 `archive/c01-handoff-20260715`；该调整没有合并无关历史或 force push。今后新任务统一从 `main` 建分支。
...
`git rev-parse HEAD` 与 `git ls-remote origin main` 输出的 SHA 必须一致。若私有仓库返回 404 或认证失败，请仓库所有者在 GitHub 邀请准确用户名；不要共享 Token/密码，也不要关闭 TLS。
...
如果有不属于自己的未提交修改，先停止并找文件负责人，不能覆盖。确认工作区干净后，更新默认主线并从最新 `main` 建立新的能力分支：
...
历史上的恢复分支与旧 `main` 没有共同祖先，因此 C01 当时没有创建虚假 PR。C02 已把可信恢复历史提升为现在的默认 `main`，旧历史继续由归档分支保存。所有后续功能分支都从当前 `main` 创建，并以 `main` 为 Draft PR 的 base。

GitHub 网页创建 Draft PR 的实际步骤：

1. push 功能分支后打开 `https://github.com/lshhhhhhhhhh10/BidRadar-X/pulls`。
2. 点击 **New pull request**。
3. `base` 选择 `main`，`compare` 选择刚推送的功能分支；例如 `feat/r01-ccgp-collector`。
4. 先检查 Commits 和 Files changed 只包含本能力内容。
5. 填完仓库七项模板后，使用 **Create draft pull request**，不要创建为可立即合并状态。
6. 邀请另一名队员审查；审查、测试和门禁通过后才由约定负责人合并到 `main`。
...
当前实际接手分支和 PR base 都是 `main`。两个 `archive/*` 分支只用于历史核验，不作为开发起点，也不接受功能 PR。
...
```powershell
git fetch origin --prune
git worktree add ..\BidRadar-X-r01 -b feat/r01-ccgp-collector main
git worktree add ..\BidRadar-X-docs -b docs/c02-example main
git worktree list
```
